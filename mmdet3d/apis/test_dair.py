...
def single_gpu_test_dair(model,
    ...
    for idx, data in enumerate(data_loader):
        ...
        #print(data['gt_bboxes_3d'].data[0])
        #print(data['gt_labels_3d'].data[0])
...
def vis_pred_label_2(pred, label, str, veh_c_s_world, inf_c_s_world, out_dir):
        ...
        t_folder = os.path.join(out_dir, 'vis_results_dair', 'truth')
        p_folder = os.path.join(out_dir, 'vis_results_dair', 'predict')
        if not osp.exists(t_folder):
            os.makedirs(t_folder)
        if not osp.exists(p_folder):
